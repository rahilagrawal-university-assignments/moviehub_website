...
class Controller(object):
    ...
    def update_survey(self, s_id, status, q_string=None):
        self.model.update_survey(s_id, status, q_string)
...
class LibraryModel(object):
    ...
    def update_survey(self, s_id, status, q_string=None):
        if q_string:
            query = "UPDATE SURVEYS SET QUESTIONS = ?, OPEN = ? \
                     WHERE SURVEY_ID = ?"
            payload = (q_string, status, s_id)
        else:
    ...
    def add_completion(self, user_id, s_id):
        query = "INSERT INTO SURVEY_COMPLETIONS (USER_ID, SURVEY_ID) \
                 VALUES (?, ?)"
