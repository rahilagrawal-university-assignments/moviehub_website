...
from functions import read_csv, find_question
import sqlite3
...
for c in model.get_everything("COURSES"):
    ...
    courses_list[c[0]] = new_course
    if new_course.code not in display_courses:
        display_courses.append(new_course.code)

# initialises questions list
for q in model.get_everything("QUESTIONS"):
    question = Question(q[1], q[0], q[2], q[3])
    questions_list[q[0]] = question

# initialises list of surveys
for s in model.get_everything("SURVEYS"):
    c = model.search_courses(s[2])[0]
    q_list = [find_question(e) for e in model.survey_questions(s[1])]
    survey = Survey(s[1], s[2], c[0], c[1], s[3],
                    q_list, s[0])
    surveys_list[s[0]] = survey
